...
metadata_sources = [# required
    ...
    { type = "license", spdx = ["MIT"], copyright = ["My Other Copyright"], files_covered = [
        ...
        { type = "msbuild", include = "path/to/project2.csproj", runtime = "win-x64" },
        { type = "nuget" }
    ] },
...
1. MSBuild coverage pattern:
   ```
   { type = "msbuild", include = "path/to/project.csproj/sln", runtime = "win-x64"}
   ```
   where
   - `type` is always `msbuild`,
   - `include` is the path to the project or solution file to cover,
   - `runtime` (optional) is the target runtime to pass to MSBuild when reading the properties; useful for cases when you publish app in self-contained mode.
   ...
   - `**/$(TargetName).exe` (only for projects that have `$(UseAppHost) = true` and their `$(TargetFileName)` has `.dll` extension).
...
- `spdx` _(optional for ignored items)_ is the SPDX expression of the license used for the file. It might be a `LicenseRef-<SomeId>` for custom licenses. This is currently partially supported by the tool, see #TODO for more information.
